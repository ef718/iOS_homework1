...
class ViewController: UIViewController {
    ...
    */
    func findTheDuplicates(fileContents: String) {
        ...
        var lineArray = fileContents.componentsSeparatedByString("\n")
        
        var wordsArray: [String] = []
        
        for line in lineArray {
            var lineWords = line.componentsSeparatedByString(" ")
            for word in lineWords {
                wordsArray.append(word.lowercaseString.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceAndNewlineCharacterSet()))
            }
        }
        ...
        let sortedWords = Array(counter.keys).sorted(<)
